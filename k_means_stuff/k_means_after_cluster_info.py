...
def color_similarity(image, data):
    ...
    with h5py.File(data, 'r') as df, h5py.File(color_image_path, 'r') as color:
...
def composition_similarity(image, data):
    ...
    with h5py.File(data, 'r') as df, h5py.File(comp_image_path, 'r') as img:
...
def similar_art(image, weight, data):
    ...
    # print('color_averages: ', color_averages, ' and comp_averages: ', comp_averages)
    ...
    with h5py.File(data, 'r') as df:
        ...
        img_color_url = df[f'metadata/{color_match_index}/url'][()]
        response = requests.get(img_color_url)
        image_color_array = np.array(bytearray(response.content), dtype=np.uint8)
        img_color = cv2.imdecode(image_color_array, cv2.IMREAD_COLOR)

        img_comp_url = df[f'metadata/{comp_match_index}/url'][()]
        response2 = requests.get(img_comp_url)
        image_comp_array = np.array(bytearray(response2.content), dtype=np.uint8)
        img_comp = cv2.imdecode(image_comp_array, cv2.IMREAD_COLOR)
        
        img_overall_url = df[f'metadata/{overall_match_index}/url'][()]
        response3 = requests.get(img_overall_url)
        image_overall_array = np.array(bytearray(response3.content), dtype=np.uint8)
        img_overall = cv2.imdecode(image_overall_array, cv2.IMREAD_COLOR)
...
test_path = '/Users/greysonmeyer/Desktop/Erdos_Work/k_means_stuff/tester.h5'
