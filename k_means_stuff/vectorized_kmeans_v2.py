...
def compute_initial_edge_density(image):
    """
    Compute an initial edge density to estimate target density.
    """
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, 50, 150)  # Use fixed thresholds for a rough estimate
    edge_density = np.sum(edges > 0) / edges.size  # Proportion of edge pixels
    return edge_density
def evaluate_canny(image, minVal, maxVal):
    gray = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    edges = cv2.Canny(gray, int(minVal), int(maxVal))  # convert thresholds to integers
    return edges
# Objective function for optimization thresholds
def optimize_canny(thresholds, image, target_density):
    minVal, maxVal = thresholds
    edges = evaluate_canny(image, minVal, maxVal)
    edge_density = np.sum(edges > 0) / edges.size  # Proportion of edge pixels
    return abs(edge_density - target_density)  # Minimize difference to target density    
...
def composition_columns(image):
    ...
    #  Compute adaptive target density
    initial_edge_density = compute_initial_edge_density(image)
    # Scale target_density based on the initial edge density
    # You can tweak the scaling factor and offset as per your application
    target_density = 0.005 + 0.5 * initial_edge_density  # Example scaling logic
    # Initial thresholds and bounds
    x0 = [30, 255]  # Initial guess for thresholds
    bounds = [(0, 150), (150, 300)]  # Allowable range for thresholds
    #  optimization
    result = minimize(
        optimize_canny,
        x0=x0,
        bounds=bounds,
        args=(image, target_density),
        method='POWELL'
    )
    #  the best thresholds
    best_minVal, best_maxVal = result.x
    # Visualize the result
    edges = evaluate_canny(image, best_minVal, best_maxVal)
...
def display_art(image:np.array, weight:float, art_type:set, 
                data_file:str = "https://github.com/BotanCevik2/Project-Leonardo/raw/refs/heads/main/resized_images_cluster_fix_fin_3.parquet"):
